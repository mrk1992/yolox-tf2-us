...
class YoloDatasets(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i in range(index * self.batch_size, (index + 1) * self.batch_size):  
            ...
            #   No random augmentation of data during validation
            #---------------------------------------------------#
            if self.mosaic:
    ...
    def get_random_data_with_Mosaic(self, annotation_line, input_shape, max_boxes=100, hue=.1, sat=1.5, val=1.5):
        ...
        for line in annotation_line:
            ...
            # Place the pictures corresponding to the positions of the four split pictures
            dx = place_x[index]
            dy = place_y[index]
            new_image = Image.new('RGB', (w,h), (128,128,128))
            new_image.paste(image, (dx, dy))
        ...
        # Perform color gamut transformation
        hue = self.rand(-hue, hue)
        sat = self.rand(1, sat) if self.rand()<.5 else 1/self.rand(1, sat)
        val = self.rand(1, val) if self.rand()<.5 else 1/self.rand(1, val)
        ...
        # Do further processing on the box
